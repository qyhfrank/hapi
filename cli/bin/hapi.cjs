...
const RELEASE_URL = 'https://github.com/qyhfrank/hapi/releases';
...
function getBinaryPath(platformName = platform, archName = arch) {
    const pkgName = `@frankqing/hapi-${platformName}-${archName}`;
...
function reportMissingPlatformPackage(platformName = platform, archName = arch, log = console.error) {
    const platformPackage = `@frankqing/hapi-${platformName}-${archName}`;
    ...
    log(`  npm install -g @frankqing/hapi --registry=${OFFICIAL_NPM_REGISTRY}`);
    log('');
